...
<?php foreach($stores as $store):?>
      <?php if($store->usageType->list_Name ==='Storage'):?>
 <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
                <div class="card shadow-sm h-100 border-0 rounded property-card-sm">
                    <!-- Image -->
                    <?php if ($store->document_url): ?>
                        <img src="<?= Yii::getAlias('@web/' . $store->document_url) ?>" 
                             alt="Property Image" 
                    ...
                    <div class="card-body d-flex flex-column p-3">
                        <!-- Property Name -->
                        <h6 class="card-title font-weight-bold text-truncate"><?= Html::encode($store->property_name) ?></h6>

                        <!-- Usage Type -->
                        <p class="mb-3 small">
                            Usage Type: <?= Html::encode($store->usageType ? $store->usageType->list_Name : 'N/A') ?>
                        </p>
                        
                        <!-- Ownership -->
                        <p class="mb-3 small">
                            Ownership: <?= Html::encode($store->propertyOwnerShip ? $store->propertyOwnerShip->list_Name : 'N/A') ?>
                        </p>

                        <!-- Buttons -->
                        <div class="mt-auto d-flex justify-content-between">
                            <a href="<?= \yii\helpers\Url::to(['property/document', 'id'=>$store->id]) ?>" class="btn btn-view btn-sm shadow-sm d-flex align-items-center">
                                <i class="fas fa-eye icon-sm mr-1"></i> View
                            </a>
                            <a href="<?= \yii\helpers\Url::to(['property/update', 'id'=>$store->id]) ?>" class="btn btn-edit btn-sm shadow-sm d-flex align-items-center">
